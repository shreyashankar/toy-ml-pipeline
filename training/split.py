...
@register(
    ...
    output_vars=["train_output_path", "test_output_path"],
)
def main():
    ...
    input_files = [
        io.get_output_path(os.path.join("features", month))
    ...
    train_df = pd.concat(
        [
            io.load_output_df(os.path.join("features", month))
    ...
    train_output_path = io.save_output_df(
        train_df, f"{component_prefix}/train", dev=False
    )
    test_output_path = io.save_output_df(
        test_df, f"{component_prefix}/test", dev=False
    )
